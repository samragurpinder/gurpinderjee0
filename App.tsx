...
import { User, AppView, Achievement } from './types.ts';
import { createNewUserDocument, getUserData, saveUserData } from './services/dataService.ts';
import { staticQuotes, achievementDefinitions, initialTopics } from './constants.ts';
...
const TopicTracker = lazy(() => import('./components/TopicTracker.tsx'));
const TestTracker = lazy(() => import('./components/TestTracker.tsx'));
const Lectures = lazy(() => import('./components/Lectures.tsx'));
const Notes = lazy(() => import('./components/Notes.tsx'));
const Reports = lazy(() => import('./components/Reports.tsx'));
const CoachingLog = lazy(() => import('./components/CoachingLog.tsx'));
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const migrateLocalToFirestoreIfNeeded = async (uid: string) => {
            ...
            try {
                ...
                // Whether migrated or not, the check is done. Remove local data to prevent re-checks.
        ...
        const unsubscribe = onAuthStateChanged(auth, async (userAuth) => {
            try {
                if (userAuth) {
                    await migrateLocalToFirestoreIfNeeded(userAuth.uid);
    ...
    if (!currentUser || !user) {
        return (
            <Suspense fallback={<div className="h-screen w-screen flex items-center justify-center bg-background"><div className="animate-spin rounded-full h-32 w-32 border-t-2 border-b-2 border-primary"></div></div>}>
                <LoginPage />
            </Suspense>
        );
    }
            ...
            <div className="flex h-screen bg-background text-text-primary">
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 pb-20 md:pb-8">
                        <Suspense fallback={<div className="h-full w-full flex items-center justify-center"><div className="animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-primary"></div></div>}>
                            {renderView()}
                        </Suspense>
